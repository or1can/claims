...
- `echoed-claims.py` (Project A) and `split-claims.py` (Project B) solve the same restatement problem independently — n-gram-run vs whole-normalized-line — not a fork of each other. Project C separately evaluated adopting Project A's tool (Apache-2.0, so unlike Project B's it could be pointed at Project C's tree directly) and declined it on merit: it's "deliberately verbatim-only," and Project C's actual restatement failures that session were paraphrase (different wording, same fact) rather than verbatim duplication — it "would have caught none of them." Where Project C has hit verbatim duplication in practice, it has answered with one canonical constant the duplicated sites both reference, rather than a checker. Recorded here because it bears directly on how much a restatement check like this can be expected to catch (see ticket 05's resolution).
