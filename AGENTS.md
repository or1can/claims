...
current when a check changes. `claims.toml` excludes the tree from four
checks: `check-file-refs` and `check-cli-flags` for the hypothetical
paths and superseded commands it names, `stale-claims` because prose
nobody will touch again can only ever look staler, and `restatement`
because a retracted line still asserted there will never be reconciled.
`judgment-agent` still sweeps it.
